...
int main(void)
{
    ...
    const int ch3='C';
    char *text1="\nThis is the test text #1 and integer: ";
    char *text2="\nThis is the test text #2 and integer: ";
    char text3[256];
    ShmLink link1,link2,link3;
    int size1,size2,err,i=0,res3=0;

    size1=strlen(text1)+1;
    size2=sizeof(int);
    ...
    for(i=0;i<20;i++)
    {
        if( (err=shm_write(&link1,text1,0,size1))<0 )
        {
            fprintf(stderr,"[%s:%d] Error #%d for link:\n%s",
                __FILE__,__LINE__,err,shm_strerror(&link1));
            return -2;
        }
        if( (err=shm_write(&link1,&i,size1,size2))<0 )
        {
            fprintf(stderr,"[%s:%d] Error #%d for link:\n%s",
                __FILE__,__LINE__,err,shm_strerror(&link1));
            return -2;
        }
        if( (err=shm_write(&link2,text2,0,size1))<0 )
        {
            fprintf(stderr,"[%s:%d] Error #%d for link:\n%s",
                __FILE__,__LINE__,err,shm_strerror(&link2));
            return -2;
        }
        if( (err=shm_write(&link2,&i,size1,size2))<0 )
        {
            fprintf(stderr,"[%s:%d] Error #%d for link:\n%s",
                __FILE__,__LINE__,err,shm_strerror(&link2));
            return -2;
        }
        if( (err=shm_read(&link3,text3,0,size1))<0 )
        {
            fprintf(stderr,"[%s:%d] Error #%d for link:\n%s",
                __FILE__,__LINE__,err,shm_strerror(&link3));
            return -2;
        }
        if( (err=shm_read(&link3,&res3,size1,size2))<0 )
        {
            fprintf(stderr,"[%s:%d] Error #%d for link:\n%s",
                __FILE__,__LINE__,err,shm_strerror(&link3));
            return -2;
